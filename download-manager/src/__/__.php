...
class __
{
    ...
    static function remote_get($url, $headers = [])
    {
        ...
        if (is_array($response)) {
            ...
            $content = __Messages::error($response->get_error_message(), -1);
        return $content;
    }


    function registration_url(){
        $id = get_option('__wppmsg_register_url', 0);
        if($id > 0) {
    ...
    static function login_url($redirect = ''){
        $id = get_option('__wppmsg_login_url', 0);
        if($id > 0) {
            $url = get_permalink($id);
            if ($redirect != '')
    ...
    static function lostpassword_url(){
        return add_query_arg(array('action' => 'lostpassword'), __::login_url());
    }

    static function logout_url($redirect = ''){
        $logout_url = home_url("/?wppmsg_logout=".wp_create_nonce(NONCE_KEY));
    ...
    function mnemonicNumberFormat($number, $plus = true){
        if($number > 1000000){
