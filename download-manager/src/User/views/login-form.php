    ...
    <div id="wpdmlogin" <?php if(wpdm_query_var('action') == 'lostpassword') echo 'class="lostpass"'; ?>>
        ...
        <form name="loginform" id="loginform" action="" method="post" class="login-form" >
            ...
            <div class="row login-form-meta-text text-muted mb-3" style="font-size: 10px">
                <div class="col-md-5"><label><input class="wpdm-checkbox" name="rememberme" type="checkbox" id="rememberme" value="forever" /><?php _e( "Remember Me" , "private-message" ); ?></label></div>
                <div class="col-md-7 text-right"><a class="color-blue" href="<?php echo add_query_arg(['action' => 'lostpassword'], $_SERVER['REQUEST_URI']); ?>"><?php _e( "Forgot Password?" , "private-message" ); ?></a></div>
            </div>

            <div class="row">
                <div class="col-md-12"><button type="submit" name="wp-submit" id="loginform-submit" class="btn btn-block btn-primary btn-lg"><i class="fas fa-user-shield"></i> &nbsp;<?php _e( "Login" , "private-message" ); ?></button></div>
                <?php if(isset($regurl) && $regurl != ''){ ?>
                    <div class="col-md-12"><br/><a href="<?php echo $regurl; ?>" class="btn btn-block btn-link btn-xs wpdm-reg-link  color-primary"><?php _e( "Don't have an account yet?" , "private-message" ); ?> <i class="fas fa-user-plus"></i> <?php _e( "Register Now" , "private-message" ); ?></a></div>
                <?php } ?>
            </div>
...
<script>
    jQuery(function ($) {
        ...
        $('#loginform').submit(function () {
            $('#loginform-submit').html("<i class='fa fa-spin fa-sync'></i> <?php _e( "Logging In..." , "private-message" ); ?>").attr('disabled', 'disabled');
            WPPMSG.blockUI('#loginform');
            $(this).ajaxSubmit({
                error: function(error) {
                    WPPMSG.unblockUI('#loginform');
                    $('#loginform').prepend("<div class='alert alert-danger' data-title='<?php _e( "LOGIN FAILED!" , "private-message" ); ?>'>"+error.responseJSON.message+"</div>");
                ...
                success: function (res) {
                    ...
                    if (!res.success) {
                        ...
                        $('#loginform').prepend("<div class='alert alert-danger' data-title='<?php _e( "LOGIN FAILED!" , "private-message" ); ?>'>"+res.message+"</div>");
                        $('#loginform-submit').html(llbl).removeAttr('disabled');
                        <?php if((int)get_option('__wpdm_recaptcha_loginform', 0) === 1 && get_option('_wpdm_recaptcha_site_key') != ''){ ?>
                        try {
                            grecaptcha.reset();
