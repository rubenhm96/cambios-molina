...
<div id="reviews" class="restaurant-listings-Reviews">
	<div id="comments">
		<h2 class="restaurant-listings-Reviews-title"><?php
				...
				printf( esc_html( _n( '%1$s reseña de %2$s', '%1$s reseñas de %2$s', 4, 'wp-restaurant-listings' ) ), esc_html( $count ), '<span>' . get_the_title() . '</span>' );
		...
		<?php else : ?>
			...
			<p class="restaurant-listings-noreviews"><?php _e( 'Aún no hay reseñas.', 'wp-restaurant-listings' ); ?></p>
		...
		<div id="review_form_wrapper">
			<div id="review_form">
				<?php
					...
					$comment_form = array(
						'title_reply'          => have_comments() ? __( 'Añadir una reseña', 'wp-restaurant-listings' ) : sprintf( __( 'Se el primero en opinar sobre &ldquo;%s&rdquo;', 'wp-restaurant-listings' ), get_the_title() ),
						'title_reply_to'       => __( 'Dejar una respuesta a %s', 'wp-restaurant-listings' ),
						'title_reply_before'   => '<span id="reply-title" class="comment-reply-title">',
						'title_reply_after'    => '</span>',
						'comment_notes_after'  => '',
						'fields'               => array(
							'author' => '<p class="comment-form-author">' . '<label for="author">' . esc_html__( 'Nombre', 'wp-restaurant-listings' ) . ' <span class="required">*</span></label> ' .
						...
						'label_submit'  => __( 'Enviar', 'wp-restaurant-listings' ),
						'logged_in_as'  => '',
						'comment_field' => '',
					);
						...
						$comment_form['comment_field'] = '<div class="comment-form-rating"><label for="rating">' . esc_html__( 'Tu calificación', 'wp-restaurant-listings' ) . '</label><select name="rating" id="rating" aria-required="true" required>
					...
					$comment_form['comment_field'] .= '<p class="comment-form-comment"><label for="comment">' . esc_html__( 'Tu opinión', 'wp-restaurant-listings' ) . ' <span class="required">*</span></label><textarea id="comment" name="comment" cols="45" rows="8" aria-required="true" required></textarea></p>';
