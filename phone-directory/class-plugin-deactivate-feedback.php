...
if( ! class_exists( 'SBD_Usage_Feedback') ) {
	
	class SBD_Usage_Feedback {
		...
		public function filter_action_links( $links ) {
			...
			if( isset( $links['deactivate'] ) && $this->include_goodbye_form ) {
				...
				$deactivation_link = str_replace( '<a ', '<div class="wpb-goodbye-form-wrapper"><span class="wpb-goodbye-form" id="wpb-goodbye-form-' . esc_attr( $this->plugin_name ) . '"></span></div><a onclick="javascript:event.preventDefault();" id="wpb-sbd-goodbye-link-' . esc_attr( $this->plugin_name ) . '" ', $deactivation_link );
		...
		public function goodbye_ajax() {
			...
			if( is_array( $form['options'] ) ) {
				...
				*/
				$html .= '</p><div id="wpb_sbd_additional_content" style=""><label for="wpb-goodbye-reasons">' . esc_html( $form['email'] ) .'</label><br><input type="email" name="wpb-goodbye-email" id="wpb-goodbye-email" value="'.get_option('admin_email').'" /> (Optional)';
				...
				jQuery(document).ready(function($){
					 $('input[type=radio]').on('change', function() { 
						if($(this).val()=='Deactivating Temporarily' || $(this).val()=='Upgrading to Pro'){
							$('#wpb_sbd_additional_content').hide();
						}else{
							$('#wpb_sbd_additional_content').show();
						}
						
					 });

					$("#wpb-sbd-goodbye-link-<?php echo esc_attr( $this->plugin_name ); ?>").on("click",function(){
						// We'll send the user to this deactivation link when they've completed or dismissed the form
						var url = document.getElementById("wpb-sbd-goodbye-link-<?php echo esc_attr( $this->plugin_name ); ?>");
